...
import { FETCH_SETTING, ROW_KEY, PAGE_SIZE, DEFAULT_BEFORE_FETCH } from '../const';
...
interface ActionType {
  ...
  setHisSearch: (data: Recordable) => void;
  searchHistory: Recordable;
}
...
export function useDataSource(
  ...
  async function fetch(opt?: FetchParams) {
    ...
    try {
      ...
      const searchFormParams = useSearchForm ? getFieldsValue() : {};
      // const searchHis = unref(searchHistory);
      // console.log('searchHis', searchHis);
      // setHisSearch(searchFormParams);

      let params: Recordable | boolean = merge(
        pageParams,
        searchFormParams,
        // searchHis,
        searchInfo,
      ...
      if (DEFAULT_BEFORE_FETCH && isFunction(DEFAULT_BEFORE_FETCH)) {
        params = (await DEFAULT_BEFORE_FETCH(params)) || params;
      }
