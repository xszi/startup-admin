<script lang="tsx">
  import { CSSProperties, onMounted, PropType, watch } from 'vue';
  ...
  import { isNil, pick, set } from 'lodash-es';
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const getValues = computed(() => {
        ...
        const labelField = unref(getComponentProps)?.labelField || 'label';
        const valueField = unref(getComponentProps)?.valueField || 'value';
      ...
      function initEditableCell() {
        if (props.record) {
          ...
          /* eslint-disable  */
          if (!props.record.onCancelEdit) {
            props.record.onCancelEdit = () => {
              isArray(props.record?.cancelCbs) && props.record?.cancelCbs.forEach((fn) => fn());
            };
          }
          /* eslint-disable */
          if (!props.record.onSubmitEdit) {
          ...
          >
            <div class="cell-content" title={this.column.ellipsis ? this.getValues ?? '' : ''}>
              {this.column.editRender
                ...
                : !isNil(this.getValues) // 处理为零的时候导致的渲染失败
                ? this.getValues
                : '\u00A0'}
            </div>
...
<style lang="less">
  ...
  .edit-cell-rule-popover {
    display: none; // 隐藏错误提示 pop 改为输入框标红
