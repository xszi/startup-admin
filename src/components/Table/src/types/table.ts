...
export interface TableTabPane extends TabPaneProps {
  name?: string
  key: string
...
export interface BasicTableProps<T = any> {
  ...
  // 搜索区域和按钮区域显示在一行，用于小于或等于两个搜索块
  searchInline?: boolean
  // 搜索区域和标题显示在一行，搜索区域位于右侧
  simpleSearch?: boolean | string
  // 表单配置
...
export interface BasicColumn extends ColumnProps<Recordable> {
  ...
  //
  flag?: 'INDEX' | 'DEFAULT' | 'CHECKBOX' | 'RADIO' | 'ACTION' | 'CLIPBOARD'
  customTitle?: VueNode
