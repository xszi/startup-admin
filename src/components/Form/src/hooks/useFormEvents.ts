...
export function useFormEvents({
  ...
  function _setDefaultValue(data: FormSchema | FormSchema[]) {
    ...
    for (const [field, [startTimeKey, endTimeKey]] of fieldMapToTime) {
      ...
      currentFieldsValue[field] = [
        dateUtil(currentFieldsValue[startTimeKey]),
        dateUtil(currentFieldsValue[endTimeKey]),
      ];
    }
  ...
  async function handleSubmit(e?: Event): Promise<void> {
    ...
    try {
      ...
      //TODO 特殊情况，ApiTreeSelect 在 form 重置情况下，会出现 outOfDate true 的错误，antd-vue form 自带判断先做个 workaround
      if (error?.errorFields?.length === 0 && error?.outOfDate) {
        const res = handleFormValues(error?.errorFields?.values);
        emit('submit', res);
        return;
      }
